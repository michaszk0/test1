    ...
    if (page.find('[data-role="header"]').length == 0) {
        ...
        var headerGrid = '<div class="ui-grid-b ui-responsive">' +
	                        '<div class="ui-grid-b">' +
                                ...
                                '</div>' +
                                '<div class="ui-block-c"><img style="height: 35px;float: right;margin-right: 14px;margin-top: 11px;" src="img/icon.png"/></div>' +
                            '</div>' +
                        '</div>';
        ...
        for(var i=0; i < allLeaves.length; i++){
            ...
            htmlList += '<div class="ui-grid-b">'
                        + '<div class="news">'
                            ...
                            + '</div>'
                            + '<div class="newsLineText">'
                                + '<p style="font-weight: bold;margin: 0 0 4px 0;">' + leave.date.substring(0,10) + ': ' + leave.user_name + ' - ' + leave.type + '</p>'
                            + '</div>'
                        + '</div>'
                    + '</div>'
        }
...
app.setAbsence = function (params){
    ...
    if ($('#addAbsenceButton').attr('data-status') == 'setAbsence') {
        params.userId = app.getFromLocalStorage('userLogged');
